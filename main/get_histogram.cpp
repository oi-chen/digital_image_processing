...
void MainWindow::get_historgam(bool flag,bool pos){
    if(pos&&image_changed.empty()){
    ...
    if(pos)
        cvtColor(image_changed,image_changed,COLOR_RGB2BGR);
    if(!flag){
        Mat hist,grayImage=pos?image_changed:image;//初始化
        int channels[]={0};
        ...
        Mat hist_b,hist_g,hist_r,srcImage=pos?image_changed:image;//初始化
